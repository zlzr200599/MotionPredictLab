...
class VectorNetDataset(Dataset):
    ...
    def __getitem__(self,index):
        ...
        data['Map'] = map_set # lane graphs, each lane <-> graph,
        data['Mapfeature'] = map_feature # lane nodes coordinates

        data['Agent'] = graph # the center agent track
        data['Agentfeature'] = features # interval 0.1 second track point coordinates
...
if __name__ == "__main__":
    # dataset = MyDataset(raw_dir='./train/forecasting_val_v1.1')
    dataset = VectorNetDataset('./train/forecasting_train_head_1000')
    import time
    start_time = time.time()
    # for i, data_dict in enumerate(dataset):
    #     print(f"{i} | {time.time()-start_time:8.2f}")
    from dgl.dataloading import GraphDataLoader
    train_dataloader = GraphDataLoader(dataset)
    for i, gd in enumerate(train_dataloader):
        print(f"{i} | {time.time()-start_time:8.2f}")